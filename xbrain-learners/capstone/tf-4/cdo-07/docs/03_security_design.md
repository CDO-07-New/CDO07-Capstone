...
```mermaid
graph TD
    subgraph EXTERNAL["Môi trường bên ngoài (Public Network)"]
    ...
    subgraph VPC["VPC (10.0.0.0/16) — us-east-1"]
        subgraph PRIV_SUB["Private Subnet (10.0.1.0/24) — Khép kín hoàn toàn"]
            ...
            APP_SG["ECS App SG (tf4-cdo07-app-sg)\n- Mock Services (payment-gw, ledger-svc, fraud-detection)\n- AI Engine"]
            
            LAMBDA_SG["Lambda SG (tf4-cdo07-lambda-sg)\n- Transformer\n- Window Feeder\n- Cost CB / Fail-Open Fallback"]
            
            VPCE["VPC Endpoints (tf4-cdo07-vpce-sg)\n- Interface: SSM, SNS, AMG, ECR, CW, TS, KDS"]
            
            INFLUX_SG["InfluxDB SG (tf4-cdo07-influxdb-sg)\n- Timestream for InfluxDB"]
        end
    end

    subgraph AWS["AWS Managed Services Network (Private)"]
        AWS_SERVS["AWS Services\n(KDS, S3 Gateway, SSM, SNS, CloudWatch, AMG)"]
    end
    ...
    CLIENT -->|"SSM Port Forwarding (HTTPS 443)"| VPCE
    VPCE -->|"Forward (HTTPS 443)"| ALB
    
    APP_SG & LAMBDA_SG -->|"HTTPS 443 (Gọi dự đoán /v1/predict)"| ALB
    ALB -->|"HTTP 8080 (Forward to AI Engine)"| APP_SG
    
    APP_SG & LAMBDA_SG -->|"HTTPS 443 (AWS Private API Calls)"| VPCE
    VPCE -->|"AWS PrivateLink (Nội bộ)"| AWS_SERVS
    
    LAMBDA_SG -->|"TCP 8086 (Ghi/Đọc Metrics)"| INFLUX_SG
    
    AWS_SERVS -.->|"Event Source Mapping / SNS Trigger"| LAMBDA_SG & L_SLACK
    L_SLACK -->|"HTTPS 443 (Slack Alerts Outbound)"| SLACK
```

> **Đồng bộ hạ tầng**: Sơ đồ trên phản ánh đúng cấu trúc mạng thực tế được thiết lập đồng bộ với tài liệu hạ tầng [`02_infra_design.md`](02_infra_design.md) và sơ đồ kiến trúc [`CDO7.drawio.png`](images/CDO7.drawio.png). Hệ thống được triển khai khép kín hoàn toàn trong một Subnet duy nhất là `Private Subnet (10.0.1.0/24)` và Application Load Balancer là **Internal ALB** (sử dụng **Path-based routing**: `/v1/predict` chuyển tiếp tới AI Engine, `/*` chuyển tiếp tới Mock Services), không sử dụng Internet Gateway hay NAT Gateway để đảm bảo chuẩn bảo mật Zero-Trust. Pipeline xử lý dữ liệu: **Mock Services → Kinesis Data Streams → Lambda Transformer (PII DROP + Schema Whitelist) → Timestream for InfluxDB** (HTTP Query API `/api/v2/query`). Lambda Transformer đọc trực tiếp từ KDS qua **event source mapping** (không qua Firehose). Luồng `k6 → ALB` (qua SSM tunnel) và `Mock Services → ALB /v1/predict` kết thúc tại **AI Engine (ECS Fargate)** sau khi được ALB định tuyến.

### 1.2 Các Nhóm Bảo mật (Security Groups)

| `tf4-cdo07-alb-sg` | HTTPS 443 từ `tf4-cdo07-vpce-sg` (SSM Port Forwarding Session của k6 Load Generator); HTTPS 443 từ `tf4-cdo07-app-sg` (Mock Services → `/v1/predict`); HTTPS 443 từ `tf4-cdo07-lambda-sg` (Window Feeder → `/v1/predict`) | HTTP 8080 → `tf4-cdo07-app-sg` (AI Engine) | Application Load Balancer (Internal) — được quản lý bởi `terraform-aws-modules/alb` |
| `tf4-cdo07-app-sg` | HTTP 8080 từ `tf4-cdo07-alb-sg` | HTTPS 443 → `tf4-cdo07-vpce-sg` (VPC Endpoints); HTTPS 443 → `tf4-cdo07-alb-sg` (Mock Services gọi `/v1/predict`) | ECS Tasks: Mock Services (3 tasks: payment-gw, ledger-svc, fraud-detection) & AI Engine |
| `tf4-cdo07-lambda-sg` | (Không inbound — KDS event source mapping trigger Lambda Transformer; EventBridge Scheduler trigger Lambda Window Feeder; SNS trigger Lambda Fail-Open Fallback; AWS Budgets trigger Lambda CB) | HTTPS 443 → `tf4-cdo07-vpce-sg` (VPC Endpoints); HTTPS 443 → `tf4-cdo07-alb-sg` (Window Feeder → AI Engine `/v1/predict`); TCP 8086 → `tf4-cdo07-influxdb-sg` | Lambda Transformer, Lambda Window Feeder, Lambda CB, Lambda Fail-Open Fallback |
| `tf4-cdo07-vpce-sg` | HTTPS 443 từ `tf4-cdo07-app-sg` (ECS tasks) & `tf4-cdo07-lambda-sg` | HTTPS 443 → `tf4-cdo07-alb-sg` (Chuyển tiếp traffic từ SSM Port Forwarding của k6) | Tất cả Interface VPC Endpoints (SSM, SNS, AMG/Grafana, ECR, CW, TS/Timestream, KDS) |
| `tf4-cdo07-influxdb-sg` | TCP 8086 từ `tf4-cdo07-lambda-sg` (Lambda Transformer ghi qua HTTP API; Window Feeder truy vấn) | (Không outbound cần thiết) | Amazon Timestream for InfluxDB instance |
...
| **S3** | Gateway | Lambda Window Feeder và AI Engine đọc baseline models, ghi audit logs — Gateway Endpoint ưu tiên (không tốn phí, tốc độ cao) |
...
*   **Lớp Lưu trữ Time-series (Amazon Timestream for InfluxDB)**:
    ...
    *   **Cơ chế cô lập**:
        ...
        *   **Cô lập truy vấn (Logical/Query Isolation)**: Khi Lambda Window Feeder hoặc Amazon Managed Grafana truy vấn dữ liệu, hệ thống tự động chèn thêm điều kiện lọc theo tenant vào câu lệnh InfluxQL/Flux (ví dụ: `SELECT * FROM "service-metrics" WHERE "tenant_id" = 'payment-gw-prod'`).
...
*   **Điểm tiếp nhận bên ngoài (External Entry Point) — k6 Load Generator**: Để duy trì thiết kế mạng khép kín hoàn toàn của Internal ALB (không sử dụng Internet Gateway), k6 Load Generator (External) thực hiện load test thông qua cơ chế **AWS SSM Port Forwarding Session (HTTPS 443)** bắc cầu qua SSM VPC Endpoint. Cổng Internal ALB áp dụng chính sách bảo mật `ELBSecurityPolicy-TLS13-1-2-2021-06`, bắt buộc TLS 1.2+ (khuyến nghị TLS 1.3). ALB thực hiện **SSL Termination** tại cổng 443 và áp dụng **Path-based routing** để định tuyến lưu lượng: các yêu cầu dự đoán `/v1/predict` (HTTP POST) được forward dạng plain HTTP qua cổng 8080 tới AI Engine (ECS Fargate), còn các yêu cầu khác (`/*`) được forward tới Mock Services. Đường truyền từ ALB tới các task chạy hoàn toàn trong mạng nội bộ khép kín.
*   **Luồng Mock Services → ALB `/v1/predict`**: Ba Mock Services (ECS Fargate) gọi ALB endpoint `POST /v1/predict (timeout=5s)` qua HTTPS (từ `App SG` tới `ALB SG`). Toàn bộ luồng này nằm trong Private Subnet, không ra Internet.
