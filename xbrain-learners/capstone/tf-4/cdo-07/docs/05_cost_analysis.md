...
| --- | --- | --- | --- |
| **Compute** (ECS Fargate - AI Engine + 3 mock services) | **$67.22**/tháng | AI Engine 0.5 vCPU/1GB + 3 × Mock 0.5 vCPU/0.5GB, chạy 24/7 — 730h/tháng | **$22.41** |
...
| **Event Streaming** (Kinesis Data Streams On-Demand) | **~$28.50**/tháng | Lambda Transformer ghi trực tiếp vào Timestream InfluxDB write API (không cần Firehose) | $9.50 |
...
| **Total / tenant / month** | **$305.38** | | **$101.79** |

### 1.1 Architecture Insights: Phân tích Cost Model

Mức chi phí **$101.79/tenant/tháng** (tương đương run-rate **$305.38/tháng** cho toàn hệ thống) được định hình bởi các quyết định kiến trúc cốt lõi nhằm đáp ứng yêu cầu khắt khe của Fintech Client về độ trễ, tính cách ly và bảo mật:

*   **Amazon Timestream for InfluxDB ($116.40/tháng - chiếm ~38%)**: Đây là chi phí cố định (fixed cost) lớn nhất cho instance `db.influx.medium` Single-AZ. Mặc dù chi phí cao, InfluxDB cung cấp năng lực lưu trữ time-series tối ưu và hỗ trợ ngôn ngữ Flux/InfluxQL để Window Feeder Lambda thực hiện các phép tính trượt thời gian $\ge 2$ giờ và forward-fill lấp lỗ hổng dữ liệu gần như realtime, tránh ops overhead tự vận hành.
*   **VPC Endpoints ($50.40/tháng - chiếm ~16%)**: Việc triển khai 8 VPC Endpoints (Interface Endpoints cho KDS, InfluxDB, CW, SSM, SNS, ECR, AMG và S3 Gateway Endpoint) là sự đánh đổi có chủ đích (Calculated Trade-off) để đạt bảo mật tuyệt đối SOC2/Zero-Trust, chặn toàn bộ phí Data Processing đắt đỏ của NAT Gateway.
*   **Compute ($67.22/tháng - chiếm ~22%)**: Bao gồm ECS Fargate chạy AI Engine `tf4-ewma-stl-v1` (0.5 vCPU / 1GB RAM) và 3 mock services (mỗi service 0.5 vCPU / 0.5GB RAM). Tất cả 4 tasks chạy 24/7 trong suốt thời gian test (730h/tháng): vCPU cost = 4 × 0.5 × $0.04048 × 730 = **$59.10**, Memory cost = (1GB + 3×0.5GB) × $0.004445 × 730 = **$8.12**, tổng **$67.22**.
*   **Kinesis Data Streams On-Demand (~$28.50/tháng)**: Lambda Transformer ghi dữ liệu trực tiếp vào Timestream InfluxDB write API, không cần Firehose, đảm bảo khả năng hấp thụ Sudden Spike mà không mất dữ liệu.
...
*   **Fixed Base Cost (Cố định)**: ~$256.27/tháng (InfluxDB DB, VPC Endpoints, ALB, AI Engine task, Grafana workspace).
*   **Variable Cost per Tenant (Biến phí/Tenant)**: ~$16.37/tháng (Mock services compute, Kinesis ingestion, S3 logs).

| Tenant count | Monthly total cost | Avg per-tenant | Ghi chú kiến trúc |
| --- | --- | --- | --- |
| **3 (Current)** | **$305.38** | **$101.79** | Bị chi phối mạnh bởi fixed base infra (InfluxDB instance, ALB, VPC Endpoints). |
| 10 | ~$420.00 | $42.00 | Chi phí cố định bắt đầu được khấu hao; Kinesis tăng nhẹ. |
| 50 | ~$1,075.00 | $21.50 | Đạt điểm hiệu quả chi phí cao; Mock services auto-scale và Kinesis tự phân shard. |
| 200 | ~$3,530.00 | $17.65 | Tiệm cận target NFR ban đầu; cần giám sát giới hạn dung lượng và hiệu năng truy vấn của InfluxDB. |
...
*Dự báo chi phí chạy thử nghiệm 2 tuần của capstone là **$152.69** (bằng 50% run-rate tháng).*

| Service | Forecast | Actual | Delta |
| --- | --- | --- | --- |
| Compute (ECS Fargate - AI Engine + Mock services) | $33.61 | $X | ±X% |
...
| Streaming & Functions (KDS + Lambda + ECR + SNS) | $16.75 | $X | ±X% |
| **Total** | **$152.69** | **$X** | **±X%** |
...
*   **Nguy cơ cấu trúc (Architectural Risk)**: Hệ thống đang đặt AWS Budgets alert ở mức **$165** (80% của $200 hard cap) cho kỳ capstone 2 tuần. Với chi phí dự phóng thực tế là **$152.69** (76.3% budget utilization), buffer an toàn còn lại là **$47.31** (~31.2%).
*   **Hành động phòng thủ**: Cost circuit breaker qua SSM Parameter Store (công tắc `inference_enabled`) sẽ tự động chuyển sang chế độ `false` qua Lambda trigger khi chi phí chạm ngưỡng $200, chuyển hướng luồng dữ liệu sang Fail-open static thresholds (CPU>30%, Mem>50%) để ngăn chặn bill spike.
*   **Per-tenant quota enforced via**: Rate-limit middleware giới hạn 600 req/min/tenant; EventBridge rule throttling; và Kinesis shard partition key theo `tenant_id`.
...
*   [`02_infra_design.md`](02_infra_design.md) - Chi tiết hạ tầng Kinesis + Timestream for InfluxDB với run-rate **$305.38/tháng**.
